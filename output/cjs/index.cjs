...
var require_templates = __commonJS({
  "node_modules/chalk/source/templates.cjs"(exports2, module2) {
    ...
    function buildStyle(chalk16, styles4) {
      const enabled = {};
    ...
    module2.exports = (chalk16, temporary) => {
      ...
      temporary.replace(TEMPLATE_REGEX, (m, escapeCharacter, inverse, style, close, character) => {
        if (escapeCharacter) {
          ...
          chunk = [];
          chunks.push(styles4.length === 0 ? string : buildStyle(chalk16, styles4)(string));
          styles4.push({ inverse, styles: parseStyle(style) });
        } else if (close) {
          if (styles4.length === 0) {
            throw new Error("Found extraneous } in Chalk template literal");
          }
          chunks.push(buildStyle(chalk16, styles4)(chunk.join("")));
          chunk = [];
          styles4.pop();
        } else {
...
var require_source = __commonJS({
  "node_modules/chalk/source/index.cjs"(exports2, module2) {
    ...
    var chalk16 = Chalk();
    chalk16.supportsColor = stdoutColor2;
    chalk16.stderr = Chalk({ level: stderrColor2 ? stderrColor2.level : 0 });
    chalk16.stderr.supportsColor = stderrColor2;
...
__export(index_exports, {
  ...
  jsonGetters: () => json_handlers_exports,
  loadPrismaSchema: () => loadPrismaSchema,
  mutationHandlers: () => mutation_handlers_exports,
  ...
  renderGetters: () => render_handlers_exports,
  useHelper: () => useHelper,
...
var import_internals3 = __toESM(require("@prisma/internals"), 1);
...
// src/modules/field-relation-collector.ts
var { getDMMF: getDMMF3 } = import_internals3.default;
...
var chalkFactory = (options) => {
  const chalk16 = (...strings) => strings.join(" ");
  applyOptions(chalk16, options);
  Object.setPrototypeOf(chalk16, createChalk.prototype);
  return chalk16;
};
...
var chalk3 = createChalk();
var chalkStderr = createChalk({ level: stderrColor ? stderrColor.level : 0 });
var source_default = chalk3;
...
var getBGColorFunction = (color) => isHex(color) ? source_default.bgHex(color) : source_default[camelCase2(["bg", color])];
...
// src/modules/field-relation-logger.ts
var import_internals4 = __toESM(require("@prisma/internals"), 1);
var import_fs3 = __toESM(require("fs"), 1);
var { getDMMF: getDMMF4 } = import_internals4.default;
...
var PrismaQlFieldRelationLogger = class {
  ...
  buildModelTrees(rootModel, relations, maxDepth, depth = 0, visitedModels = /* @__PURE__ */ new Set()) {
    ...
    for (const relation of modelRelations) {
      ...
      const isSelfRelation = relation.modelName === relation.relatedModel;
      const selfRelationIcon = isSelfRelation ? import_chalk4.default.yellow("\u{1F501}") : "";
      let keyInfo = import_chalk4.default.gray("[-]");
      if (relation.foreignKey) {
        const direction = relation.relationDirection === "backward" ? "\u2190" : "\u2192";
        keyInfo = `[FK: ${import_chalk4.default.blue(relation.foreignKey)} ${direction} ${import_chalk4.default.green(relation.referenceKey || "id")}]`;
      } else if (relation.relationTable) {
        keyInfo = `[M:N via ${import_chalk4.default.yellow(relation.relationTable)}]`;
      }
      if (relation.relationTable && relation.relationTable !== relation.modelName) {
        ...
        table[relation.relationTable][`\u2192 ${import_chalk4.default.yellow(relation.modelName)}:${import_chalk4.default.cyan(relation.fieldName)} [FK: ${import_chalk4.default.blue(relation.foreignKey || "?")} \u2192 ${import_chalk4.default.green(relation.referenceKey || "?")}]`] = {};
        table[relation.relationTable][`\u2192 ${import_chalk4.default.yellow(relation.relatedModel)}:${import_chalk4.default.cyan(relation.inverseField)} [FK: ${import_chalk4.default.blue(relation.foreignKey || "?")} \u2192 ${import_chalk4.default.green(relation.referenceKey || "?")}]`] = {};
      }
      const constraints = relation?.constraints?.length ? `Constraints: ${import_chalk4.default.magenta(relation.constraints.join(", "))}` : "";
      const isList = (relationType === "1:M" || relationType === "M:N") && !relation?.foreignKey;
      let relationLabel = `\u2192 ${import_chalk4.default.yellow(relation.relatedModel + (isList ? "[]" : ""))}:${import_chalk4.default.cyan(name)} ${relationAlias} ${import_chalk4.default.red(relationType)} ${keyInfo} ${constraints} ${selfRelationIcon}`;
    ...
    trees.push({ [import_chalk4.default.bold(rootModel)]: table });
    for (const relation of modelRelations) {
  ...
  generateRelationTreeLog(rootModel, maxDepth = 1, relations) {
    ...
    let output = `${import_chalk4.default.green.bold("\u{1F4CA} Relation Tree Statistics")}
`;
    output += `${import_chalk4.default.yellow("Model:")} ${import_chalk4.default.bold(rootModel)}
`;
    output += `${import_chalk4.default.cyan("Max Depth:")} ${import_chalk4.default.bold(maxDepth)}
`;
    output += `${import_chalk4.default.blue("Related Models:")} ${import_chalk4.default.bold(stats.uniqueModels)}
`;
    output += `${import_chalk4.default.magenta("Total Relations:")} ${import_chalk4.default.bold(stats.totalRelations)}
`;
    output += `${import_chalk4.default.redBright("Direct Relations:")} ${import_chalk4.default.bold(stats.directRelations)}
`;
    let treeOutput = "";
    ...
    }).map((r) => import_chalk4.default.gray(r)).join("\n");
    const relsList = `${import_chalk4.default.white.bold("\u{1F517} Direct Relations")}
${results}`;
...
var highlightPrismaSchema = new import_prismalux2.PrismaHighlighter();
var PrismaQlProvider = class {
  ...
  async dryMutation(input) {
    ...
    try {
      ...
      console.log("\u2705", import_chalk5.default.gray(displayCommand), import_chalk5.default.green(`Dry run success!`));
    } catch (e) {
    ...
    console.log("\u2705", import_chalk5.default.gray(displayCommand), import_chalk5.default.green(`Validation success!`));
  ...
  async mutation(input, options = {}) {
    ...
    try {
      ...
      const displayCommand = `${parsedCommand.action} ${parsedCommand.command} > `;
      console.log("\u2705", import_chalk5.default.white(displayCommand), import_chalk5.default.green("Mutation success"));
    } catch (e) {
      throw new Error(`Modification failed: ${e.message}`);
    }
    if (options.save) {
...
var highlightPrismaSchema2 = new import_prismalux3.PrismaHighlighter();
var getEnums = (prismaState, data) => {
  ...
  if (options?.raw) {
    ...
    const parsed = (0, import_prisma_ast2.printSchema)(schema);
...
var import_prismalux4 = require("prismalux");
var highlightPrismaSchema3 = new import_prismalux4.PrismaHighlighter();
var getModel = (prismaState, data) => {
  ...
  const hSchema = highlightPrismaSchema3.highlight((0, import_prisma_ast3.printSchema)(schema));
...
var import_prismalux5 = require("prismalux");
var import_chalk11 = __toESM(require_source(), 1);
var highlightPrismaSchema4 = new import_prismalux5.PrismaHighlighter();
var getModels = (prismaState, data) => {
  ...
  const highlightedSchema = highlightPrismaSchema4.highlight((0, import_prisma_ast4.printSchema)(schema));
...
var getJsonModel = (prismaState, data) => {
  ...
  return response.result({
    model,
    schema: (0, import_prisma_ast5.printSchema)(schema),
...
var import_prisma_ast6 = require("@mrleebo/prisma-ast");
var getJsonModels = (prismaState, data) => {
  ...
  return response.result({
    total: modelCount,
    models,
    schema: (0, import_prisma_ast6.printSchema)(schema)
  });
};
...
var import_prisma_ast7 = require("@mrleebo/prisma-ast");
var addField = (prismaState, data) => {
  ...
  let parsed;
  try {
    parsed = (0, import_prisma_ast7.getSchema)(sourceField);
  } catch (error) {
...
var import_prisma_ast8 = require("@mrleebo/prisma-ast");
var addModel = (prismaState, data) => {
  ...
  try {
    ...
    try {
      parsed = (0, import_prisma_ast8.getSchema)(sourceModel);
    } catch (error) {
...
var deleteRelation = (prismaState, data) => {
  ...
  if (options?.relationName) {
    console.log(import_chalk12.default.yellow(`Relation name is specified. Attempting to remove relation by name`));
  ...
  if (!fieldA && !fieldB) {
    console.log(import_chalk12.default.yellow(`No fields specified. Attempting to remove all relations between ${modelA} and ${modelB}`));
...
var import_prisma_ast9 = require("@mrleebo/prisma-ast");
var updateField = (prismaState, data) => {
  ...
  try {
    parsed = (0, import_prisma_ast9.getSchema)(sourceField);
  } catch (error) {
...
var import_prisma_ast10 = require("@mrleebo/prisma-ast");
var addGenerator = (prismaState, data) => {
  ...
  try {
    parsed = (0, import_prisma_ast10.getSchema)(sourceModel);
  } catch (error) {
    return response.error(`Invalid block provided. Error parsing model: ${error.message}`);
  }
...
var import_prisma_ast11 = require("@mrleebo/prisma-ast");
...
var updateGenerator = (prismaState, data) => {
  ...
  if (prismaBlock) {
    ...
    console.log(import_chalk13.default.yellow(`Warning: generator block provided. Generator ${generatorName} will be replaced`));
    builder.drop(prevGenerator.name);
    const sourceModel = `generator ${generatorName} {
        ${prismaBlock}
    }`;
    try {
      parsed = (0, import_prisma_ast11.getSchema)(sourceModel);
    } catch (error) {
      return response.error(`Invalid block provided. Error parsing model: ${error.message}`);
    }
    const schema = prismaState.builder.getSchema();
...
var getGenerators = (prismaState, data) => {
  ...
  const result = sections.map((section) => {
    return boxen(import_chalk14.default.bold(section.name) + "\n" + section.table, { padding: 1, borderStyle: "bold" });
  }).join("\n");
  return response.result(
    `
...
var getDB = (prismaState, data) => {
  ...
  return response.result(boxen(`${import_chalk15.default.gray("Provider")}: ${provider}
${import_chalk15.default.gray("URL")}: ${prevUrl}`, { padding: 1, textAlignment: "left", margin: 1, borderStyle: "double" }));
};
