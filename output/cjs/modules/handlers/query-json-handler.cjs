...
var require_templates = __commonJS({
  "node_modules/chalk/source/templates.cjs"(exports2, module2) {
    ...
    function buildStyle(chalk5, styles4) {
      const enabled = {};
...
module.exports = __toCommonJS(query_json_handler_exports);
...
var getJsonModel = (prismaState, data) => {
  ...
  return response.result({
    model,
    schema: (0, import_prisma_ast.printSchema)(schema),
...
var getJsonModels = (prismaState, data) => {
  ...
  return response.result({
    total: modelCount,
    models,
    schema: (0, import_prisma_ast2.printSchema)(schema)
  });
};
...
var getEnumRelations = (prismaState, data) => {
  ...
  const formattedModels = formatColumns(relations.map((rel) => `${import_chalk4.default.hex("#11FF00")("\u2022")} ${import_chalk4.default.bold(rel.model.name)} -> ${import_chalk4.default.bold(rel.field.name)}`), columns);
  const stats = `${import_chalk4.default.white("\u{1F4CA} Total relations:")} ${import_chalk4.default.white.bold(total)}`;
