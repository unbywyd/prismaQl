...
var get_enums_exports = {};
__export(get_enums_exports, {
  getEnums: () => getEnums
});
module.exports = __toCommonJS(get_enums_exports);
...
var getEnums = (prismaState, data) => {
  ...
  if (!enums.length) {
    return response.result(import_chalk2.default.yellow(`\u26A0 No enums found`));
  }
  const totalEnums = enums.length;
  const statistic = `\u{1F4CC} Enums in schema: ${import_chalk2.default.bold(totalEnums)}`;
  if (options?.raw) {
    const schema = {
      type: "schema",
      list: enums
...
${rawOutput}`, {
  ...
  enums.forEach((e) => {
    list.push({
      name: import_chalk2.default.white.bold(e.name),
      options: e.enumerators?.filter((e2) => e2.type == "enumerator").map((en) => import_chalk2.default.green(en?.name)) || []
    });
  });
