...
var getFields = (prismaState, data) => {
  ...
  if (!fields.length) {
    return response.result(import_chalk2.default.yellow(`\u26A0 No fields found in model ${modelName}`));
  }
  ...
  if (!fields.length) {
    return response.result(import_chalk2.default.yellow(`\u26A0 No fields found in model ${modelName} that match filters`));
  }
  const idField = fields.find((f) => f.attributes?.some((attr) => attr.name === "id"))?.name;
  const table = new import_cli_table3.default({
    head: [
      import_chalk2.default.bold("Field Name"),
      import_chalk2.default.bold("Type"),
      import_chalk2.default.bold("Required"),
      import_chalk2.default.bold("Array"),
      import_chalk2.default.bold("Relation"),
      import_chalk2.default.bold("Attributes")
    ],
  ...
  fields.forEach((field) => {
    let name = import_chalk2.default.greenBright(field.name);
    const type = import_chalk2.default.blueBright(field.fieldType);
    const required = field.optional ? import_chalk2.default.redBright("No") : import_chalk2.default.greenBright("Yes");
    const array = field.array ? import_chalk2.default.yellowBright("Yes") : import_chalk2.default.gray("No");
    let hasRelation = field.attributes?.some((attr) => attr.name === "relation");
    let relation = hasRelation ? import_chalk2.default.magentaBright("Yes") : import_chalk2.default.gray("No");
    ...
    const attributes = field.attributes?.filter(
      ...
      return `@${attr.name}`;
    }).join(", ") || import_chalk2.default.gray("None");
    for (const rel of relations) {
      if (rel.modelName === model.name) {
        if (rel.fieldName === field.name) {
          relation = import_chalk2.default.magentaBright(rel.relationName);
          hasRelation = true;
        }
        if (rel.foreignKey === field.name) {
          relation = import_chalk2.default.magenta(`${rel.relationName} (FK)`);
        }
      }
    }
    if (hasRelation) {
    ...
    if (field.name == idField) {
      name = `${import_chalk2.default.bgGreenBright.black(field.name)} (ID)`;
    }
    table.push([name, type, required, array, relation, attributes]);
  });
  const totalFoundFields = fields.length;
  const statistic = `
    \u{1F4CC}Fields in model: ${import_chalk2.default.bold(modelName)}
    Total fields found: ${import_chalk2.default.bold(totalFoundFields)}
    Fields with relations: ${import_chalk2.default.bold(relationFields)}
    `;
