    ...
    module2.exports = (chalk4, temporary) => {
      ...
      temporary.replace(TEMPLATE_REGEX, (m, escapeCharacter, inverse, style, close, character) => {
        if (escapeCharacter) {
          ...
          chunk = [];
          chunks.push(styles4.length === 0 ? string : buildStyle(chalk4, styles4)(string));
...
var get_model_exports = {};
__export(get_model_exports, {
  getModel: () => getModel
});
module.exports = __toCommonJS(get_model_exports);
...
var getModel = (prismaState, data) => {
  ...
  const hSchema = highlightPrismaSchema.highlight((0, import_prisma_ast.printSchema)(schema));
  let output = `${import_chalk3.default.bold.whiteBright("Model:")} ${import_chalk3.default.greenBright(model.name)}
`;
  output += `${import_chalk3.default.whiteBright("Relations:")} ${totalRelations > 0 ? `${import_chalk3.default.greenBright(totalRelations)} relations` : import_chalk3.default.redBright("No relations")}

`;
  const maxFieldLength = Math.max(...fields.map((f) => f.name.length), 5);
  const maxTypeLength = Math.max(...fields.map((f) => f.type.length), 4);
  output += import_chalk3.default.underline("Unique Fields:\n");
  output += fields.map((field) => {
    const fieldName = field.isId ? import_chalk3.default.bold.red(field.name) : field.isUnique ? import_chalk3.default.bold.yellow(field.name) : import_chalk3.default.white(field.name);
    const fieldType = field.isRelation ? import_chalk3.default.cyan(field.type) : import_chalk3.default.blueBright(field.type);
    return `${fieldName.padEnd(maxFieldLength + 2)} ${fieldType.padEnd(
      maxTypeLength + 2
    )}`;
  }).join("\n");
  output += "\n\n";
  output += import_chalk3.default.underline("Schema:") + hSchema;
