...
import { enumsToSchema, fieldsToSchema, relationsToSchema } from "../../renders.js";
import relationLogger from "../../relation-logger.js";
import { renderModelNames } from "./render-model-names.js";
...
export const provideQueryRenderHandlers = (instance) => {
    instance.registerCommandHandler("GET_MODEL_NAMES", async (prismaData) => {
        ...
        const models = parsedSchema.list
            ...
            .map(model => model);
        return renderModelNames(models);
    });
    instance.registerCommandHandler("GET_RELATIONS", async (prismaData, command, manager) => {
        const { parsedSchema } = prismaData;
    ...
    instance.registerCommandHandler("GET_MODELS", async (prismaData, command) => {
        const { parsedSchema } = prismaData;
        const modelNames = command?.models || [];
        const models = parsedSchema.list
            .filter(item => item.type === "model")
            .map(model => model);
        if (modelNames.length) {
            return renderModels(models.filter(model => modelNames.includes(model.name)));
        }
        return renderModels(models);
    });
    instance.registerCommandHandler("GET_MODEL", async (...args) => {
        const [prismaData, command] = args;
    ...
    instance.registerCommandHandler("GET_RELATION_FIELDS", async ({ builder }, command) => {
        const model = builder.findByType("model", { name: command.model });
        if (!model) {
            throw new Error(`Model "${command.model}" not found.`);
        }
