...
export type QueryCommand =
    ...
    | { type: "GET_FIELD"; model: string; field: string, attribute?: string } // Optional field filter
    | { type: "GET_RELATIONS"; model: string }
    | { type: "GET_ENUMS" }
    | { type: "GET_ENUM"; enum: string }
    | { type: "VALIDATE_SCHEMA" }
    | { type: "PRINT" }
    | { type: "GET_MODEL_NAMES" } // New command to get all model names
    | { type: "GET_ENUM_NAMES" } // New command to get all enum names
    | { type: "GET_RELATION_TREE"; model: string, depth?: number }; // New command to get relation tree
...
export const QueryCommandTypes = [
    ...
    "GET_ENUM_NAMES",
    "GET_RELATION_TREE",
] as QueryCommand["type"][];

/**
 * Mutation command types for modifying the Prisma schema.
 */
export type MutationCommand =
    | { type: "ADD_MODEL"; model: string; fields: FieldDefinition[] }
    | { type: "REMOVE_MODEL"; model: string }
    | { type: "ADD_FIELD"; model: string; field: FieldDefinition }
    | { type: "REMOVE_FIELD"; model: string; field: string }
    | { type: "UPDATE_FIELD"; model: string; field: string; definition: string }
    | { type: "ADD_RELATION"; model: string; relation: RelationDefinition }
    | { type: "REMOVE_RELATION"; model: string; relation: string }
    | { type: "ADD_ENUM"; enum: string; values: string[] }
    | { type: "REMOVE_ENUM"; enum: string };
...
export function parseCommand(commandStr: string): Command {
    ...
    if (match) {
        ...
        const fields: FieldDefinition[] = fieldParts.map((fp) => {
            ...
            const field: FieldDefinition = { name: fieldName, type: fieldType, attributes: attributes.length ? attributes : undefined };
    ...
    if (match) {
        ...
        const relation: RelationDefinition = { name: relationName, definition: relationDefinition };
